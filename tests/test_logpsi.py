...
def fermiflow(depth, spsize, tpsize, Nf, L, n, dim):
    from ferminet import FermiNet
    def flow_fn(x):
        model = FermiNet(depth, spsize, tpsize, Nf, L, True)
        return model(x)
...
"""
def test_logpsi():
    depth, spsize, tpsize, Nf, L = 3, 16, 16, 5, 1.234
    rs = 1.0
    n, dim = 14, 3
    flow, s, x, params = fermiflow(depth, spsize, tpsize, Nf, L, n, dim)
...
def test_kinetic_energy():
    ...
    """
    batchsize, n, dim = 10, 2, 3
    L = 1.234
    rs = 1.0

    identity = hk.transform(lambda x: (x[n:], jnp.ones((n, n))) )
    s = jnp.array( np.random.uniform(0., L, (batchsize, n, dim)) )
    x = jnp.array( np.random.uniform(0., L, (batchsize, n, dim)) )
    params = identity.init(key, x)

    logpsi = make_logpsi(identity, L, rs)
    _, logpsi_grad_laplacian = make_logpsi_grad_laplacian(logpsi)
    grad, laplacian = logpsi_grad_laplacian(x, params, s, key)
    assert grad.shape == (batchsize, n, dim)
    assert laplacian.shape == (batchsize,)

    kinetic = -laplacian - (grad**2).sum(axis=(-2, -1))
    print("kinetic energy:", jnp.mean(kinetic)/(rs**2*n))
    
    def vep(s, x):
        rij = jnp.reshape(s, (n, 1, dim)) - jnp.reshape(x, (1, n, dim)) 
        rij = rij - L*jnp.rint(rij/L)
        rij = jnp.linalg.norm(rij, axis=-1)
        return jnp.sum(-1/rij)/2

    print("potential energy:", jnp.mean(jax.vmap(vep)(s,x)))

    print (kinetic) 
    potential = jax.vmap(vep, (0, 0), 0)(s,x)
    print (potential)
    print (kinetic + 4*potential)


   
def test_laplacian():
    """ Check the two implementations of logpsi laplacian are equivalent. """
    depth, spsize, tpsize, Nf, L = 2, 4, 4, 5, 1.234
    n, dim = 14, 3
    flow, x, params = fermiflow(depth, spsize, tpsize, Nf, L, n, dim)
